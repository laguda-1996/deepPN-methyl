...
pos_train_fa = 'pos_m6A_test.fasta'
neg_train_fa = 'neg_m6A_test.fasta'
...
model = CNN41_RNN(HIDDEN_NUM, LAYER_NUM, FC_DROPOUT, RNN_DROPOUT, CELL)
...
for i, color in zip(range(len(fprArray)), colors):
    plt.plot(fprArray[i], tprArray[i], lw=1.5, alpha=0.9, color='r',label=' (AUC = %0.2f)' % (ROC_aucs[i]))
...
for i, color in zip(range(len(recall_array)), colors):
    plt.plot(recall_array[i], precision_array[i], lw=1.5, alpha=0.9, color='r',label=' (AUPRC = %0.2f)' % (PR_aucs[i]))
