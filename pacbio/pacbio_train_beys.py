...
if __name__ == '__main__':
    ...
    # cnn option
    parser.add_argument("-c1_o", "--conv1_outchannel", action="store", dest='conv1_outchannel', default=[64,128], type=int, nargs='*',
                        help="")
    parser.add_argument("-c2_o", "--conv2_outchannel", action="store", dest='conv2_outchannel', default=[128,256], type=int, nargs='*',
                        help="")
    parser.add_argument("-c1_k", "--conv1_kernel", action="store", dest='conv1_kernel', default=[7,5], type=int, nargs='*',
                        help="")
    parser.add_argument("-c2_k", "--conv2_kernel", action="store", dest='conv2_kernel', default=[5,3], type=int, nargs='*',
                        help="")

    # rnn option
    parser.add_argument("-rnntype", "--rnn_type", action="store", dest='rnn_type', default='LSTM', type=str,
                        help="[capital] LSTM(default), GRU")
    parser.add_argument("-hidnum", "--hidden_num", action="store", dest='hidden_num', default=128, type=int, nargs='*',
                        help="rnn size")
    ...
    parser.add_argument("-batch", "--batch_size", action="store", dest='batch_size', default=128, type=int,
                        help="batch size")
    ...
    Conv1_outchannel = args.conv1_outchannel
    Conv2_outchannel = args.conv2_outchannel
    Conv1_kernel = args.conv1_kernel
    Conv2_kernel = args.conv2_kernel
    # print(type(Conv1_kernel))
    ...
    def train_evaluate(parameterization):
        ...
        model = train(CNN41_RNN(HIDDEN_NUM, LAYER_NUM, RNN_DROPOUT, FC_DROPOUT, CELL),
