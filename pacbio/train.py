...
if __name__ == '__main__':
    ...
    for trained, valided in folds:
        ...
        # model = CNN_41(FC_DROPOUT)
        # model = RNN(FC_DROPOUT, RNN_DROPOUT, CELL, HIDDEN_NUM, LAYER_NUM)
        model = CNN41_RNN(HIDDEN_NUM, LAYER_NUM, RNN_DROPOUT, FC_DROPOUT, CELL)
        model = model.to(device)
        loss = torch.nn.CrossEntropyLoss(reduction='sum')
        # loss = loss.to(device)
        optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE, weight_decay=9.47358338162303e-10)
        # optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE, weight_decay=1.4350874722003936e-09)
        if args.optim == 'RMSprop':
            optimizer = optim.RMSprop(model.parameters(), lr=LEARNING_RATE)
        scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=9, gamma=0.4317949827760458)
        # scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=10, gamma=0.2990561112761497)
        ...
        def mytrain(model, loss, optimizer, x, y):
            ...
            # grad_clip
            torch.nn.utils.clip_grad_norm_(model.parameters(), 19)
        ...
        for i in range(EPOCH):
            ...
            for k in range(num_batches):
                ...
                output_train, y_pred_prob, y_batch, y_pred_train = mytrain(model, loss, optimizer, X_train[start:end],
                                                                         y_train[start:end])
                cost += output_train
